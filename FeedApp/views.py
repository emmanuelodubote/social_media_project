...
def friends(request):
    ...
    #If this is the first time to access the friend request page, create the first relationship with
    #the admin of the website (so the admins is friends with everyone)
    ...
    if request.method == 'POST' and request.POST.get('send_requets'):
    ...
    if request.method == 'POST' and request.POST.get('receive_requets'):
        senders = request.POST.getlist('receive_requets')
        for sender in senders:
            ...
            #Create a relationship object to access the sender's user ID to add to the friend list of the user
